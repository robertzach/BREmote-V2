// V2.5-Evo - 2026-08-28 - FM Warning Distance is now live. Range corrected to the existing one-byte telemetry ceiling (164m), with immediate then 2-second medium-pulse warnings at/above the threshold. Existing field/struct/SW_VERSION unchanged.
// V2.5-Evo - 2026-08-28 - FM Arm Window help now distinguishes trigger-independent RX lifecycle readiness from trigger-gated motor/steering authority. Text only; no field/struct/SW_VERSION change.
...
{key:"fm_hold_duration_s",label:"FM Hold Time",description:"RIGHT-hold duration for the LEFT-tap → RIGHT-hold FM combo. Live and tunable: 3s (fast) to 10s (very deliberate). Default 3. This does not change armed mode selection: with trigger released, LEFT/RIGHT hold steps backward/forward through F1-F6 after 1s and every 1s while held; F0 is skipped.",group:"Follow-Me",type:"int",def:3,min:3,max:10,unit:"s"},
...
{key:"fm_warn_distance_m",label:"FM Warning Distance",description:"TX-to-RX separation at which the remote gives one medium vibration immediately and then every 2 seconds while Follow-Me remains armed and the fresh distance stays at or above this value. The warning also runs with the trigger released; it stops below the threshold, on FM disarm or on link loss. Range 50-164 m, default 150. The 164 m maximum is fixed by the existing one-byte distance telemetry.",group:"Follow-Me",type:"int",def:150,min:50,max:164,unit:"m"},
