// V2.5-Evo - 2026-08-27 - ?diag now describes a standing heading-disagreement latch as GPS-only degradation, not an FM engagement block. The compass remains withdrawn, while valid live/held COG may engage and steer FM. Read-only text change; no control/config/packet/SW_VERSION change.
...
void cmdDiag(const String& params) {
  ...
  // heading ladder while Follow-Me and FM Return may use valid GPS COG. Read-only —
  // headingDisagreeLatched() cannot set, clear or age the
  ...
  Serial.printf("heading    : compass-vs-COG disagreement latch %s\n",
                headingDisagreeLatched()
                  ? "STANDING - compass withdrawn; FM may use valid GPS COG; clears on sustained agreement, ?compasscal (full run) or ?magalign"
