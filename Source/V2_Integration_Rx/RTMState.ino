// V2.5-Evo - 2026-08-29 - A LoRa failsafe timeout is now an FM authority hold, not an FM lifecycle fault. PWM.ino already stops emitting motor pulses at the same strict failsafe_time boundary; runFmLoop additionally withdraws fm_rx_active, centres automatic steering, holds cap 0 and resets every unfinished geometry/convergence proof while preserving FM_ARMED, FM_ACTIVE or FM_RETURN and the completed separation/min-distance latches. The first recovered control packet remains held until one post-outage 2 Hz TX GPS sample arrives, preventing its still-stale timestamp from immediately reclassifying the link gap as a GPS STOPPING; that wait is bounded at 6 s, after which a persistent GPS failure regains normal fault semantics. Link recovery then resumes ACTIVE through the existing 0->target engage ramp and resumes RETURN through its reset ramp/convergence window, without STOPPING, St/buzz or re-arm. Genuine GPS/Phase-B/heading/divergence faults still end an active run. The independent 95 s unrefreshed-mode expiry remains the lost-disarm backstop. No config/packet/struct change; SW_VERSION stays 35.
...
// V2.5-Evo - 2026-07-25 - Batch A (A2+A3), RX FM only. (A2) fm_engage_dist_m is now READ: >0 sets the FM engage distance directly in metres (rope x ~1.15), 0 keeps the previous auto behaviour (kFmEngageFactor x d_follow) bit-for-bit; latch, dwell and Schmitt hysteresis untouched. (A3) FM divergence FAULT — runFmLoop() gained the upper distance bound it never had: condition 8 is a lower bound only, and runPhaseC()'s convergence check is RTM-only (called from runRtmLoop, never runFmLoop), so a wrong heading let FM steer away indefinitely. dist_m > kFmDivergeFactor(2.0) x D_engage sustained kFmDivergeMs(3000) while FM_ACTIVE now routes through the EXISTING fault path (FM_STOPPING ramp -> FM_IDLE, re-arm required, same haptic/St semantics as the sensor faults). Non-blocking first-exceed timestamp, cleared on any condition/state/data-trust change. Subtract-only: adds no throttle, extends no engagement, does not touch the deadman. All compile-time constants; no confStruct change; SW_VERSION stays 34.
...
//       heading the override is pinned at 127 and the align cap holds the throttle at 60/255;
...
static bool getRtmHeading(float* out_heading, uint8_t* out_confidence)
{
  ...
  //   updateRtmSteering() — reached only with the trigger held (RTM past its gates, or FM automatic
  //     authority via authority_ok), so again nothing prints;
...
// Stateful F1-F6 speed governor. The old stateless law multiplied the available throttle by
...
// target. GPS speed and the F4-F6 gap-derived target are filtered independently; the small deadband
...
// Catch-up reopens only after the buggy has fallen this far beyond the relevant regulation-band
// entry. The inner edge ends catch-up immediately; this outer edge prevents GPS noise from toggling
// full catch-up authority at the boundary.
static const float    kFmCatchupReentryHysteresisM = 2.0f;

// Align-phase throttle cap (60/255, ~24%). While the heading error is large the buggy
// should pivot toward the target, not drive away from it. Same value RTM's align phase uses.
static const uint8_t  kFmAlignCap            = 60;     // 0-255

// Engage ramp length. Whenever actual automatic authority starts or resumes, the throttle cap ramps
// 0 -> full over this time. A trigger-free FM_ACTIVE lifecycle edge does not start the ramp.
...
// After the first post-timeout control packet, wait for a newly decoded 2 Hz TX GPS sample before
// evaluating packet-derived sensor faults. This closes the recovery ordering gap where `last_packet`
// is fresh but rx_tx_gps_timestamp still describes the outage. Six seconds is deliberately bounded:
// failure to refresh beyond it is a real sensor fault, not an endless state-preserving link hold.
static const uint32_t kFmLinkRecoveryGraceMs = 6000;

// FAULT stop ramp. On a sensor fault (conditions 2-6) or divergence FM hands throttle back to the rider by ramping the
...
// engagements. (1) At the time, the engage ramp was 3500 ms, LONGER than the 3000 ms dwell, so the
// fault could fire before the buggy had even been given full throttle. (2) At the time, the align cap
// was 13/255 (~5%), so the buggy pivoted on the spot and the distance GREW before it started
...
//   FM_ARMED   : a mode (1-6) is selected and all monitoring runs, but the radial separation proof
//                has not completed yet. The proof does not require a held trigger. The throttle chain
//                is INACTIVE (cap 255), so the rider keeps full manual control while FM waits.
//   FM_ACTIVE  : the separation proof has completed and FM owns the lifecycle until FM_RETURN,
//                explicit disarm or fault. fm_rx_active separately says whether the held trigger has
...
//                and manual cap 255. Every automatic resume restarts the engage ramp and P+D state.
...
//   FM_STOPPING: FM was engaged and a FAULT dropped out - conditions 2-6 (Phase A/B, TX/RX GPS
//                stale, heading invalid) or divergence. Something actually broke, so autonomy ends for
...
//                LoRa timeout is different: PWM already stops at failsafe_time, while FM preserves
//                this state and its completed latches for a ramped resume when the link returns.
...
// Link loss preserves fm_state, but recovery must not judge a stale pre-outage GPS snapshot on the
// first control packet. These values keep the motorless hold active until one new TX GPS sample is
// decoded, or until kFmLinkRecoveryGraceMs expires and normal sensor-fault semantics resume.
static bool          fm_link_hold_active       = false;
static unsigned long fm_link_hold_tx_gps_ms    = 0;
static unsigned long fm_link_recovery_start_ms = 0;
...
// Last valid F1-F6 station used by the active controller. Changing station while FM_ACTIVE can
// move the target across the rider (especially F4 <-> F6), so the next control tick deliberately
// re-enters through the ordinary steering reset and 0->full throttle engage ramp.
static uint8_t       fm_control_mode                = 0xFF;

// Side-zone Schmitt state: true = apply the diagonal offset, false = sit directly behind.
static bool          fm_diagonal_engaged = false;

// millis() at the moment automatic authority most recently started. Drives the engage ramp.
// A trigger-free FM_ARMED -> FM_ACTIVE lifecycle edge deliberately leaves this at 0.
...
// Both normal RETURN exits lead to FM_ARMED and preserve the live F1-F6 declaration. If the trigger
...
static bool          fm_front_warning    = false;  // bit7: F4-F6 front-position warning only
...
// Actual automatic-authority eligibility, including the held-trigger deadman. FM lifecycle
// readiness is instead represented by fm_state == FM_ACTIVE even when this remains false.
...
void runFmNavigationLoop()
{
  ...
  //           [6] radial/separation warning [7] F4-F6 front-position warning (both warning-only).
  {
    uint8_t f = 0;
    FmState s = fm_state;
...
// ------------------------------------------------------------
// Front-mode helpers - F4 Front-Left, F5 Front, F6 Front-Right
// ------------------------------------------------------------
// Front modes reuse near_diag_offset_deg so no config field or packet change is needed. The stored
// field historically accepts values through 180 degrees for F1/F3; clamp only the front-mode use
// below 90 degrees so a mode named "Front" can never place its station behind the rider.
// In the clockwise-from-North bearing convention, negative is rider-left and positive rider-right.
// ------------------------------------------------------------
static bool fmIsFrontMode(uint8_t mode)
{
  return followMeIsFrontMode(mode);
}

static float fmFrontTargetOffsetDeg(uint8_t mode)
{
  return followMeFrontOffsetDeg(mode, (float)usrConf.near_diag_offset_deg);
}
...
// ------------------------------------------------------------
// fmFrontGeometry - rider-relative position for modes 4-6
// ------------------------------------------------------------
...
// the distinction a radial distance cannot make. Cross-track and relative angle are signed: right
// is positive and left negative in this clockwise bearing convention.
// A rider course is mandatory: while stationary there is no defensible meaning of "in front".
// ------------------------------------------------------------
static bool fmFrontGeometry(float* out_along_m, float* out_cross_m, float* out_relative_deg)
{
  *out_along_m      = 0.0f;
  *out_cross_m      = 0.0f;
  *out_relative_deg = 180.0f;
...
{
  // Follow radius = hard-stop radius plus smoothing band (factory default 10 + 10 = 20 m).
  ...
  if (fm_rider_course_deg < 0.0f) {
    // F4-F6 geometry is warning-only, so a missing rider course must not withdraw control. Keep the
  ...
  // ---- Modes 4-6 Front-Left / Front / Front-Right: station plus steering lookahead ----
  ...
  if (fmIsFrontMode(m)) {
    ...
    // the speed cap; steering remains at least one lookahead ahead along the rider's course.
    float steer_along_m = station_along_m + lookahead_m;
    ...
    if (isfinite(anchor_to_buggy_m) && isfinite(anchor_to_buggy_bearing)) {
      ...
      if (ahead_of_buggy_m > steer_along_m) steer_along_m = ahead_of_buggy_m;
    } else {
    ...
    float steer_distance_m = hypotf(steer_along_m, station_cross_m);
    float steer_bearing_deg = course + atan2f(station_cross_m, steer_along_m) *
        (180.0f / (float)M_PI);
    projectPoint(anchor_lat, anchor_lng, steer_bearing_deg, steer_distance_m, out_lat, out_lng);
    return;
  }
...
// ------------------------------------------------------------
// checkFmFaultConditions - the FM FAULT conditions (2-6)
// ------------------------------------------------------------
// What it does (DESIGN_FOLLOW_ME.md section 5, transient-inhibit-vs-fault classification):
//   Evaluates the five sensor FAULT conditions only. While FM_ARMED they prevent separation proof/readiness;
//   once FM_ACTIVE they end the run and require a fresh declaration. Two conditions are handled by
//   the CALLER, not here, because they are authority holds rather than lifecycle faults:
...
//     - Condition 7 (LoRa link) is the physical PWM failsafe. The caller holds authority/state work
//       before evaluating these sensors, because packet-derived GPS will naturally become stale
//       during a long radio gap and must not convert that same link hold into a sensor STOPPING.
//     - Follow warning thresholds never change steering authority, state or separation proof. Signed
//       front geometry is nevertheless a speed-governor input: invalid data cannot grant F4-F6 V-Max
//       catch-up and falls back to the normal rider-relative target. The retired configurable rider-
//       speed gate no longer participates; only the fixed <2 km/h RETURN proof can temporarily hold.
//   Like RTM, any one of these five failing means FM must not be steering. This function does NOT
...
//
// Returns: true only if all five sensor fault conditions hold.
...
static bool checkFmFaultConditions()
{
  ...
  //    missing heading source in FM means the buggy would steer blind at the ~24% align cap, which
...
// Cold-start the F1-F6 speed governor. Called at every genuine automatic-control edge and while FM
...
{
  bool outside_regulation = false;
  bool beyond_reentry     = false;

  if (fmIsFrontMode(mode)) {
    if (front_geometry_valid && isfinite(front_along_m)) {
      float station_radius_m = usrConf.min_dist_m + usrConf.followme_smoothing_band_m;
      if (station_radius_m < 0.5f) station_radius_m = 0.5f;
      float target_along_m = fmFrontTargetAlongM(station_radius_m, mode);

      float control_band_m = usrConf.followme_smoothing_band_m;
      if (control_band_m < 1.0f) control_band_m = 1.0f;

      float positive_gap_error_m = target_along_m - front_along_m;
      outside_regulation = positive_gap_error_m > control_band_m;
      beyond_reentry = positive_gap_error_m >
          (control_band_m + kFmCatchupReentryHysteresisM);
    }
  } else if (isfinite(dist_m)) {
    float d_follow_m = usrConf.min_dist_m + usrConf.followme_smoothing_band_m;
    if (d_follow_m < 0.5f) d_follow_m = 0.5f;
    outside_regulation = dist_m > d_follow_m;
    beyond_reentry = dist_m > (d_follow_m + kFmCatchupReentryHysteresisM);
  }

  bool previous = fm_speed_catchup_active;
  if (!fm_speed_catchup_init) {
    fm_speed_catchup_init   = true;
    fm_speed_catchup_active = outside_regulation;
  } else if (fm_speed_catchup_active) {
    if (!outside_regulation) fm_speed_catchup_active = false;
  } else if (beyond_reentry) {
    fm_speed_catchup_active = true;
  }

  // A phase edge changes the meaning of the speed target. Re-initialising from the live GPS speed
  // makes the new target effective immediately; in particular, leaving catch-up cannot retain a
  // one-second filtered V-Max target while the buggy is already inside the distance-control band.
  if (fm_speed_catchup_active != previous) fm_speed_gov_init = false;
...
static float fmSpeedTargetKmh(float front_along_m, uint8_t mode)
{
  ...
  if (fmIsFrontMode(mode)) {
    ...
    d_front = fmFrontTargetAlongM(d_front, mode);
...
//                          starts or resumes, so a trigger-free lifecycle edge cannot create a jump.
//
// Inputs:  dist_m - radial buggy-to-rider distance; front_along_m - signed F4-F6 forward distance;
//          front_geometry_valid - whether that signed measurement is trustworthy;
//          mode - active geometry; now - millis() for this tick
// Returns: the winning cap, 0-255
// Side effects: updates the F1-F6 speed PI/filter state.
// ------------------------------------------------------------
static uint16_t fmComputeThrottleCap(float dist_m, float front_along_m,
                                     bool front_geometry_valid,
                                     uint8_t mode, unsigned long now)
{
  ...
  bool  in_front = fmIsFrontMode(mode);

  // ---- Cap 2: approach ramp across the smoothing band ----
  // Behind modes slow as the buggy approaches the rider. F4-F6 deliberately do not apply this
  // radial ramp: when a buggy ahead is being caught, slowing it makes the front gap collapse even
  // faster. Front modes instead control the along-track gap in the speed governor below; the same
  // radial min_dist hard stop remains enforced by runFmLoop().
...
static void fmEnterIdle()
{
  ...
  fm_control_mode     = 0xFF;
  fm_link_hold_active       = false;
  fm_link_hold_tx_gps_ms    = 0;
  fm_link_recovery_start_ms = 0;
...
// FM_ARMED with the live F1-F6 declaration preserved and the separation proof cleared. Motor posture
...
void runFmLoop()
{
  ...
  if (usrConf.gps_en && usrConf.rtm_rx_enabled && diag_mode >= 1 && diag_mode <= 6) {
  ...
  // beyond the engage distance could have made FM ready without a declaration and handed steering
  // to it on the next trigger squeeze. WHAT THE FIX DOES: 0xFF now means FM_IDLE, always, and 0xFF is greater
  // than 6 so the mode gate below catches it. usrConf.followme_mode keeps exactly one job —
  ...
  // Compass.ino only refreshes that snapshot while thr_received < 25. FM automatic authority runs
  // under a HELD trigger, so about one second into control the comparison goes dormant and the 5 s dwell can
  ...
  // A live mode change does not inherit the previous station's steering derivative or throttle
  // allowance. Publish the safe values first; the normal !was_controlling path below then resets
  // P+D history and the speed governor and starts a fresh kFmEngageRampMs ramp toward F1-F6's new
  // target. The separation proof remains valid because its contract is radial and mode-independent.
  bool mode_changed_while_active =
      (fm_state == FM_ACTIVE && fm_control_mode != 0xFF && fm_control_mode != m);
  fm_control_mode = m;
  ...
  // ---- Link failsafe hold ----
  // PWM.ino independently stops motor pulses at this exact boundary. Do not turn the same event
  // into FM_STOPPING: preserve the current logical lifecycle and completed latches, but withdraw
  // every automatic output and discard unfinished proofs based on packet-derived data. This check
  // deliberately precedes sensor fault evaluation; otherwise a long link gap would eventually make
  // TX GPS stale and incorrectly relabel the link hold as a lifecycle-ending GPS fault.
  bool link_ok = followMeLinkHealthy(
      (uint32_t)last_packet, (uint32_t)now, (uint32_t)usrConf.failsafe_time);
  bool apply_link_hold = false;
  if (!link_ok) {
    if (!fm_link_hold_active) {
      fm_link_hold_tx_gps_ms = rx_tx_gps_timestamp;
    }
    fm_link_hold_active       = true;
    fm_link_recovery_start_ms = 0;
    apply_link_hold           = true;
  } else {
    fm_diag_gate_flags |= FM_LOG_GATE_LINK_OK;
    if (fm_link_hold_active) {
      if (fm_link_recovery_start_ms == 0) fm_link_recovery_start_ms = now;
      bool tx_gps_refreshed = followMeTxGpsRefreshedAfterLinkHold(
          (uint32_t)fm_link_hold_tx_gps_ms, (uint32_t)rx_tx_gps_timestamp);
      if (followMeLinkRecoveryShouldHold(
              tx_gps_refreshed, (uint32_t)fm_link_recovery_start_ms,
              (uint32_t)now, (uint32_t)kFmLinkRecoveryGraceMs)) {
        apply_link_hold = true;
      } else {
        // With a fresh sample, normal sensor gates decide immediately. With no sample after the
        // bounded grace, those same gates intentionally turn a persistent GPS failure into the
        // ordinary STOPPING path instead of keeping ACTIVE/RETURN dormant forever.
        fm_link_hold_active       = false;
        fm_link_hold_tx_gps_ms    = 0;
        fm_link_recovery_start_ms = 0;
      }
  ...
  if (apply_link_hold) {
    FollowMeLinkHoldPolicy link_hold = followMeLinkHold((uint8_t)fm_state);
    fm_diag_block_reason = FM_LOG_BLOCK_LINK;
    fm_state             = (FmState)link_hold.lifecycle_state;
    fm_rx_active         = link_hold.automatic_authority;
    rtm_steer_override   = 127;
    fm_throttle_cap      = link_hold.throttle_cap;
    fm_engage_ms         = 0;
    ...
    // Pause RETURN's runtime budget as well as its convergence proof. On the recovery tick its
    // existing engage cap starts from this recent timestamp and fm_rx_active=false cold-starts the
    // shared governor before power can be exposed again.
    if (fm_state == FM_RETURN) fm_return_start_ms = now;
    fmResetSpeedGovernor();
    return;
  }

  // ---- Evaluate the remaining conditions ----
  // DEADMAN = trigger. FAULT = sensor/heading gates. Geometry is diagnostic only;
  ...
  bool  fault_ok = checkFmFaultConditions();   // conditions 2-6 (sensor FAULT)
  ...
  // must still carry its sensor cause into STOPPING even though it is not surprise-haptic.
  uint8_t fault_block_reason = fault_ok ? FM_LOG_BLOCK_NONE : fm_diag_block_reason;
  bool  authority_ok = thr_held && fault_ok && link_ok; // final gate for actual automatic authority
  ...
  bool  front_mode           = fmIsFrontMode(m);
  bool  front_geometry_valid = false;
  float front_along_m        = 0.0f;
  float front_cross_m        = 0.0f;
  float front_relative_deg   = 180.0f;
  float front_axis_error_deg = 180.0f;
  // V2.5-Evo - 2026-07-25 - A3: sustained divergence while ACTIVE. Classed as a FAULT (same family
  // as sensor conditions 2-6), so it is routed through the SAME FM_STOPPING path below — never its own.
  ...
  // ACTIVE sensor/heading fault retains STOPPING priority and cannot be reclassified as arrival.
  ...
  if (fm_state == FM_RETURN) {
    ...
    if (!fmReturnFaultOk(now)) {
      fmReturnFaultStop(now, (FmLogBlockReason)fm_diag_block_reason,
                        "GPS/heading gate failed", thr_held);
      return;
    }
  ...
  // or the separation latch. F1-F3 retain min/min+band as warning thresholds; F4-F6 use signed
  // front position plus angular error from their selected front-left/front/front-right axis.
  ...
  if (geometry_eval_ok && front_mode) {
    front_geometry_valid = fmFrontGeometry(
        &front_along_m, &front_cross_m, &front_relative_deg);
    if (front_geometry_valid && isfinite(front_relative_deg)) {
      front_axis_error_deg = fmAngleDiff(front_relative_deg, fmFrontTargetOffsetDeg(m));
      fm_diag_front_angle_deg = front_axis_error_deg;
    }
  }
  ...
  if (!active_session) {
    ...
    if (front_mode) {
      bool front_invalid = !front_geometry_valid ||
                           (dist_m <= min_dist) ||
                           (front_along_m <= 0.0f) ||
                           (front_axis_error_deg >= usrConf.zone_angle_exit_deg);
      bool front_recovered = front_geometry_valid &&
                             (dist_m > d_follow_e) &&
                             (front_along_m > 0.0f) &&
                             (front_axis_error_deg < usrConf.zone_angle_enter_deg);
  ...
  // ---- Separation proof: one activation geometry for every F1-F6 mode ----
  ...
  if (followMeSeparationSample(fault_ok, latch_position_ok,
    ...
    } else if (!fm_sep_latched && followMeSeparationConfirmed(
                   (uint32_t)fm_sep_over_since_ms, (uint32_t)now,
                   (uint32_t)kFmSepDwellMs)) {
      fm_sep_latched = true;
  ...
  // ---- min_dist_m recoverable stop ----
  // The first trustworthy <=min sample during an engaged/proven session removes automatic steering
  // and publishes cap 0 before diagnostics. While the FM_ACTIVE lifecycle continues, trustworthy
  ...
  if (!fm_min_dist_stop_latched && active_session && thr_held && latch_position_ok &&
    ...
    rtm_steer_override       = 127;
    fm_throttle_cap          = 0;
    Serial.printf("FM [RX] MIN-DIST STOP: dist=%.1f m <= min_dist_m=%.1f m; cap 0, separation proof retained\n",
                  dist_m, min_dist);
  }
  ...
  if (geometry_eval_ok) {
    ...
    // engage ramp was 3500 ms but the dwell was only 3000 ms, so the fault could fire BEFORE the
    // buggy had finished being given throttle. Second, the align cap was 13/255 (~5%), so the buggy pivoted
    ...
    //      kFmEngageRampMs + kFmDivergeMs (1500 + 3000 = 4500 ms) after automatic authority starts.
    ...
    else if (was_controlling && authority_ok && !return_proof_wait &&
    ...
    // No trustworthy geometry this tick (GPS stale/rejected). Restart every proof
  ...
  bool lifecycle_ready = followMeLifecycleReady(
      ...
      fm_min_dist_stop_latched, diverge_fault);
  bool can_be_active = followMeAutomaticAuthority(lifecycle_ready, thr_held, link_ok);
  ...
  if (can_be_active) {
    ...
    // The lifecycle is ready/ACTIVE, but the physical deadman withholds actual motor and steering
    // authority. Proof blockers above remain visible even while the trigger is open.
  ...
  if (can_be_active) {
    ...
    if (!was_controlling) {
      ...
      fm_state = FM_ACTIVE;
      if (front_mode) {
        Serial.printf("FM [RX] %s mode %u front: dist=%.1f m along=%.1f m cross=%.1f m rel=%.1f deg target=%.1f deg error=%.1f deg rider=%.1f km/h course=%.0f\n",
                      first_engagement ? "ENGAGE" : "RESUME",
                      (unsigned)m, dist_m, front_along_m, front_cross_m, front_relative_deg,
                      fmFrontTargetOffsetDeg(m), front_axis_error_deg,
    ...
    computeFmTarget(&fm_target_lat, &fm_target_lng);       // trailing point (F1-3) or front lookahead (F4-6)
    updateFmSteering();                                    // shared P+D controller
    fm_throttle_cap = (uint8_t)fmComputeThrottleCap(
        dist_m, front_along_m, front_geometry_valid, m, now);
  }
  else {
    // ---- Not eligible to steer — classify FAULT vs transient ACTIVE inhibit vs proof-pending ARMED ----
    fm_rx_active       = false;
    ...
    // ladder to GPS COG. Only an actual condition-2..6 failure (including no valid COG/hold) or the
    // divergence fault ends an ACTIVE session.
    if ((!fault_ok || diverge_fault) && active_session) {
      // ---- FAULT (conditions 2-6, plus A3 divergence): something broke in the ACTIVE lifecycle ----
      ...
      // ---- FM_ARMED: separation proof not complete — fully manual buggy ----
