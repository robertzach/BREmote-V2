// V2.5-Evo - 2026-08-28 - Steering Influence help now documents the throttle-relative, aggregate-power-neutral T-delta/T+delta differential mix. Text only; no field/struct/SW_VERSION change. Kept byte-identical to the standalone tool.
// V2.5-Evo - 2026-08-28 - Boogie V-Max help now states that catch-up always opens speed cap 3 and this setting is the in-band PI ceiling only. Text only; no field/struct/SW_VERSION change. Kept byte-identical to the standalone tool.
// V2.5-Evo - 2026-08-28 - FM Engage help now states that the 2-second separation proof and FM_ARMED -> FM_ACTIVE lifecycle edge do not require the trigger; actual motor/steering authority still does. Text only; no field/struct/SW_VERSION change. Kept byte-identical to the standalone tool.
// V2.5-Evo - 2026-08-27 - Added Steering Reduction Start (default 50%) and Full-Throttle Steering (default 35%). They reuse the retired foiler-low-speed float and the remaining u16 reserve in place; visible fields 62->64, ConfigService rows 63->64, sizeof stays 192 and SW_VERSION stays 35. Kept byte-identical to the standalone tool.
// V2.5-Evo - 2026-08-27 - Added absolute-metre fm_diverge_dist_m to Follow-Me using the in-place renamed reserved float. Effective range is 2 x D_engage through 100 m; 0 reconstructs the old 6 x D_engage behavior and applies the 100 m cap. UI field only mirrors the unchanged 192-byte config layout; SW_VERSION stays 35. There are now 62 visible fields = 63 ConfigService rows minus the one remaining hidden reserve. Kept byte-identical to the standalone tool. [Superseded later the same day: the final reserve became Steering Reduction Start, so there are now 64 visible rows.]
// V2.5-Evo - 2026-08-28 - Follow-Me help text documents the unified stationary ACTIVE -> FM_RETURN lifecycle at every distance, immediate completion to FM_ARMED at/inside D_engage, and moving min-distance recovery. Text only; no field/struct/SW_VERSION change. Kept byte-identical to the standalone tool.
// V2.5-Evo - 2026-08-27 - Follow-Me selector extended to F4 Front Left, F5 Front and F6 Front Right. F4/F6 reuse near_diag_offset_deg. [SUPERSEDED 2026-08-28 for front distance: F4-F6 now use twice the common rear-follow radius.] UI only; no confStruct/SW_VERSION change.
...
// V2.5-Evo - 2026-08-28 - FM Return Target Speed now exposes the literal 0-50 km/h PI setpoint; zero means zero speed and the old 8 km/h firmware limit is gone.
...
<script>
// 64 user-facing RX parameters. This is every row of kCfgFields; retired standalone-RTM fields
// are not descriptors. If you add, rename or remove a kCfgFields row, this count and the array
...
{key:"steering_influence",label:"Steering Influence",description:"Throttle-relative differential strength. Steering redistributes the current throttle request as T-delta / T+delta without increasing the combined normalized motor command. 0=straight only; 100=full differential (inside motor can reach zero at full steering). Default 50.",group:"Steering",type:"int",def:50,min:0,max:100,unit:"%"},
{key:"steer_reduction_start_pct",label:"Steering Reduction Start",description:"Effective throttle above which steering authority begins to reduce. Manual steering, manual takeover during Follow-Me and automatic Follow-Me all use the same curve after throttle safety caps. At and below this value steering remains at full configured influence. Default 50%. Range 30-80%.",group:"Steering",type:"int",def:50,min:30,max:80,unit:"%"},
{key:"steer_full_throttle_pct",label:"Full-Throttle Steering",description:"Steering authority retained at 100% effective throttle. Firmware blends smoothly from full authority at Steering Reduction Start to this value at full throttle. Default 35%. Set 100% to disable throttle-dependent reduction. Range 20-100%.",group:"Steering",type:"float",def:35.0,min:20.0,max:100.0,step:1.0,unit:"%"},
...
{key:"followme_mode",label:"Follow-Me Mode",description:"Stored FM mode preference on this RX. IT IS NOT AN AUTO-ARM SOURCE: the RX arms Follow-Me only from the TX's live 0xF2 mode declaration, and an undeclared mode means IDLE — it never falls back to this value. Kept for backup/restore and documentation. 0=Disabled, 1=Near Right, 2=Behind (default), 3=Near Left, 4=Front Left, 5=Front, 6=Front Right. Every F1-F6 mode engages from the same radial FM Engage Distance proof; F4-F6 target-axis limits are warning-only.",group:"Follow-Me",type:"enum",def:2,min:0,max:6,options:[{v:0,l:"Disabled"},{v:1,l:"Near Right"},{v:2,l:"Behind (default)"},{v:3,l:"Near Left"},{v:4,l:"Front Left"},{v:5,l:"Front"},{v:6,l:"Front Right"}]},
...
{key:"rtm_align_threshold_deg",label:"FM Return Align Threshold",description:"While return heading error exceeds this angle, throttle is limited to 60/255 (about 24%) so the buggy aligns before accelerating. Range 10-90 degrees, default 45.",group:"Follow-Me",type:"int",def:45,min:10,max:90,unit:"deg"},
{key:"rtm_target_speed_kmh",label:"FM Return Target Speed",description:"Literal PI-governor speed target during FM Return. The rider trigger remains the power source. 0 commands zero speed; range 0-50 km/h. A non-zero Boogie V-Max remains an absolute ceiling. Default 4.",group:"Follow-Me",type:"float",def:4.0,min:0,max:50,step:0.5,unit:"km/h"},
{key:"boogie_vmax_in_followme_kmh",label:"Boogie V-Max",description:"Absolute ceiling for the in-band F1-F6 PI speed target. Catch-up always opens speed cap 3 to maximum rider-requested throttle, independent of this value. Once the applicable radial or signed front-gap control band is reached, rider-relative PI regulation resumes and a non-zero value is its ceiling. 0 removes the absolute in-band ceiling. A finite target reaches throttle cap 0 at target plus 2 km/h. Default 25 km/h.",group:"Follow-Me",type:"float",def:25.0,min:0,max:100,step:0.1,unit:"km/h"},
{key:"min_dist_m",label:"Min Distance",description:"Hard-stop distance while Follow-Me is active. At or below it throttle cap is 0. If distance recovers above it while the rider is moving, Follow-Me keeps the separation proof and resumes through its engage ramp. A rider stationary below 2 km/h for 2 seconds completes every FM Active lifecycle through FM Return, regardless of distance; at or inside the Return radius it immediately enters FM Armed without return motion and clears the lifecycle latches. Default 10 m.",group:"Follow-Me",type:"float",def:10.0,min:0,max:1000,step:0.1,unit:"m"},
...
{key:"zone_angle_enter_deg",label:"Zone Angle Enter",description:"Geometry Schmitt inner half-angle. F1/F3: below this angle from directly behind, the diagonal side offset is applied. F4-F6: returning inside this error from the selected front axis clears the front-position warning. It never gates engagement, steering or throttle. Default 35°.",group:"Follow-Me",type:"float",def:35.0,min:0,max:180,step:0.1,unit:"deg"},
{key:"zone_angle_exit_deg",label:"Zone Angle Exit",description:"Geometry Schmitt outer half-angle. F1/F3: above it, the diagonal side offset is dropped. F4-F6: crossing it relative to the selected front axis raises a warning but does not change steering, throttle cap, state or separation proof. The warning repeats every 3 seconds. Keep 5-15° wider than Zone Angle Enter. Default 45°.",group:"Follow-Me",type:"float",def:45.0,min:0,max:180,step:0.1,unit:"deg"},
{key:"near_diag_offset_deg",label:"Near Diag Offset",description:"Diagonal bearing offset. F1 Near Right uses -offset from behind and F3 Near Left uses +offset. F4 Front Left uses -offset from ahead, F5 Front uses 0, and F6 Front Right uses +offset. Front use is capped below 90° so it remains ahead. Default 45°.",group:"Follow-Me",type:"float",def:45.0,min:0,max:180,step:0.1,unit:"deg"},
{key:"fm_engage_dist_m",label:"FM Engage / Return Distance",description:"The one radial activation boundary for every F1-F6 mode and the FM Return arrival radius. Valid values are 0=automatic or 8-50 m. Set at least one metre beyond the tow rope. Automatic computes 1.5 x (Min Distance + Smoothing Band), with the same 8 m floor. Automatic Follow-Me requires 2 continuous seconds outside this radius, including after stationary FM Active completion entered FM Armed. The trigger is not required for this proof or the FM_ARMED to FM_ACTIVE lifecycle edge; it remains required for actual motor and automatic-steering authority. A moving Min Distance recovery retains its proof. A stationary FM Active rider enters FM Return at any trustworthy distance: outside this radius it retrieves, while at or inside it immediately completes to FM Armed without return motion. A stationary FM Armed declaration can enter Return only outside this radius, preventing a near-range state loop. Every normal Return exit clears the separation latch, preserves the declaration and requires a fresh proof outside this radius.",group:"Follow-Me",type:"float",def:0,min:0,max:50,step:0.5,unit:"m"},
{key:"fm_diverge_dist_m",label:"FM Divergence Distance",description:"Absolute distance ceiling for the sustained FM_ACTIVE divergence fault. The effective value is always raised to at least 2 x the effective FM Engage Distance and capped at 100 m. Example: with D_engage=11 m, the minimum is 22 m; entering 60 m gives a 60 m limit, while entering 15 m is raised to 22 m. 0 is only the existing-config compatibility value: it derives the previous 6 x D_engage limit and then applies the 100 m cap. A fault still requires 3 seconds without closing by more than 2 m.",group:"Follow-Me",type:"float",def:100.0,min:0,max:100,step:1,unit:"m"},
{key:"log_level",label:"Log Detail Level",description:"How much detail each log record carries. More detail means bigger records, so the on-board storage fills up faster. 0 = Automatic (DEFAULT) — behaves exactly like level 3, and is what every unit already stores, so leaving this alone changes nothing. 1 = Basic and 2 = VESC are RESERVED for a future storage optimisation (smaller records for longer sessions) and are NOT implemented yet: they are accepted and saved, but the firmware CURRENTLY LOGS AS LEVEL 3. They are not silently ignored — they simply do not save you any space yet. 3 = Developer — VESC, GPS, RTM/Follow-Me heading, steering actually applied, remote distance and LoRa link quality. 59 bytes per record. 4 = Deep — everything in Developer plus GPS sentence/COG/mux/loop diagnostics, the exact Follow-Me engage audit and heading evidence: COG live/hold/frozen states, compass snapshot age, compass-vs-COG difference and disagreement dwell. 96 bytes per record, roughly 50 min at 5 Hz or 1 h 25 min at the 3 Hz default. Use 4 when diagnosing Follow-Me; 0 or 3 for normal riding. Those figures assume storage is otherwise empty — the logger keeps a 500 KB reserve and rolls off older files. Each log FILE records its own level and record size, so changing this mid-session is safe and old 65-byte and 83-byte Deep logs remain readable. NOTE: this setting replaces the old RESERVED FM Steer Reposition slot, which no firmware ever read.",group:"Logging",type:"enum",def:0,min:0,max:4,options:[{v:0,l:"0 — Automatic (same as Developer)"},{v:1,l:"1 — Basic (reserved — currently logs as Developer)"},{v:2,l:"2 — VESC (reserved — currently logs as Developer)"},{v:3,l:"3 — Developer (full record, 59 B)"},{v:4,l:"4 — Deep (FM + heading audit, 96 B)"}]},
