...
// V2.5-Evo - 2026-08-28 - Follow-Me help text documents recoverable ACTIVE min-distance stops and the stationary-release transition to FM_ARMED. Text only; no field/struct/SW_VERSION change. Kept byte-identical to the standalone tool.
...
{key:"min_dist_m",label:"Min Distance",description:"Hard-stop distance while Follow-Me is active. At or below it throttle cap is 0. If distance recovers above it, Follow-Me keeps the separation proof and resumes through its engage ramp. If the rider remains below 2 km/h at or below it for 2 seconds, releasing the trigger enters FM Armed, restores manual throttle and clears the separation proof. Default 10 m.",group:"Follow-Me",type:"float",def:10.0,min:0,max:1000,step:0.1,unit:"m"},
...
{key:"fm_engage_dist_m",label:"FM Engage / Return Distance",description:"The one radial activation boundary for every F1-F6 mode and the FM Return arrival radius. Valid values are 0=automatic or 8-50 m. Set at least one metre beyond the tow rope. Automatic computes 1.5 x (Min Distance + Smoothing Band), with the same 8 m floor. Automatic Follow-Me requires 2 continuous seconds outside this radius, including after a stationary Min Distance handoff entered FM Armed. The trigger is not required for this proof or the FM_ARMED to FM_ACTIVE lifecycle edge; it remains required for actual motor and automatic-steering authority. A moving Min Distance recovery retains its proof. A stationary rider outside this radius can enter FM Return, which clears the separation latch. Arrival inside it enters FM_ARMED with the declaration preserved; a fresh proof outside this radius is required before automatic Follow-Me can engage again.",group:"Follow-Me",type:"float",def:0,min:0,max:50,step:0.5,unit:"m"},
