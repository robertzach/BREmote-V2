// V2.5-Evo - 2026-08-27 - F4 zone-angle descriptions updated: the angles now drive an advisory periodic vibration only and no longer gate engagement or force HOLD. UI text only; no confStruct/SW_VERSION change. Kept byte-identical to the standalone tool.
// V2.5-Evo - 2026-08-27 - Added the two now-live high-throttle steering controls: reduction start (default 50%) and full-throttle authority (default 35%). They replace the two intentionally-hidden reserved slots, so the visible field count rises 64->66 while kCfgFields/confStruct counts and sizes stay unchanged. Kept byte-identical to the standalone tool.
...
// V2.5-Evo - 2026-08-25 - Follow-Me selector extended with F4 In Front (max 4). The original already-ahead/front-cone engagement gate was superseded by the 2026-08-27 advisory-only change above. near_diag_offset sign prose corrected to match the clockwise-bearing implementation (Right negative, Left positive). UI only; no confStruct/SW_VERSION change.
...
<script>
// 66 Parameters for RX. This is every row of kCfgFields (ConfigService.ino); the two formerly
// hidden reserved slots are now the live high-throttle steering settings. If you add, rename or
// remove a kCfgFields row, this count and the array below must move with it.
...
{key:"steer_reduction_start_pct",label:"Steering Reduction Start",description:"Effective throttle above which steering authority begins to reduce. The reduction applies equally to manual steering, RTM and Follow-Me, after all throttle safety caps. At and below this value steering stays at full configured influence. Default 50%. Range 1-99%.",group:"Steering",type:"int",def:50,min:1,max:99,unit:"%"},
{key:"steer_full_throttle_pct",label:"Full-Throttle Steering",description:"Steering authority retained at 100% effective throttle. The firmware blends smoothly from full authority at Steering Reduction Start to this value at full throttle. Default 35%. Set 100% to disable throttle-dependent reduction. Range 1-100%.",group:"Steering",type:"float",def:35.0,min:1.0,max:100.0,step:1.0,unit:"%"},
...
{key:"zone_angle_enter_deg",label:"Zone Angle Enter",description:"Geometry Schmitt enter half-angle. F1/F3: below this angle from directly behind, the diagonal side offset is applied (not an engagement gate). F4: below this angle from the rider's forward course, the periodic angle warning clears. F4 engagement is not angle-gated. Default 35°.",group:"Follow-Me",type:"float",def:35.0,min:0,max:180,step:0.1,unit:"deg"},
{key:"zone_angle_exit_deg",label:"Zone Angle Exit",description:"Geometry Schmitt exit half-angle. F1/F3: above it, the diagonal is dropped. F4: above it, an advisory vibration starts and repeats every 3 s; it does not stop or block F4. Keep 5-15° wider than Zone Angle Enter. Default 45°.",group:"Follow-Me",type:"float",def:45.0,min:0,max:180,step:0.1,unit:"deg"},
